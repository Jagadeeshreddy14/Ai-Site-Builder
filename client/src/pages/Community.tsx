            ...
            {loading ? (
                ...
                <div className='py-10 min-h-[80vh]'>
                    <div className='flex items-center justify-between mb-12'>
                        <h1 className='text-3xl font-semibold text-white mb-6'>Published Projects</h1>
                        ...
                        {projects.map((project) => (
                            ...
                            >
                                ...
                                <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                                    ...
                                    <p className='text-gray-400 mt-1 text-sm '>{project.initial_prompt}</p>
                                    <div  className='flex justify-between items-center mt-6'>
                                        <span>{new Date(project.createdAt).toLocaleDateString()}</span>
                                        <div className='flex gap-3 text-white text-sm'>
                                            <button className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md  transition-colors flex items-center gap-2'><span>{project.user?.name?.slice(0, 1)}</span>
                                            {project.user?.name}
                                            </button>
                ...
                <div className='flex flex-col items-center justify-center h-[80vh] text-center gap-6'>
                    <h1 className='text-2xl font-semibold text-white'>You have no projects yet</h1>
                    <p className='text-gray-400 max-w-md'>Get started by creating a new project. Click the button below to create your first project.</p>
                    <button 
                        onClick={() => navigate('/')} 
